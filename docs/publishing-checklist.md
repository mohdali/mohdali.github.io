...
- Use `image` and `imageAlt` only when the post needs a custom social preview instead of the default site card.
- Custom social previews include accurate `imageType`, `imageWidth`, and `imageHeight`; CI fails rather than guessing dimensions.
